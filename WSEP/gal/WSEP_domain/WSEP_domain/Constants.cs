...
namespace WSEP_domain
{
    public static class Constants
    {
        ...
        private static readonly IList<string> ILLEGAL_VALUES = new ReadOnlyCollection<string> (new List<string>() { NULL, EMPTY, GUEST });

        public const string NEWLINE = "\n";
        private static readonly IList<string> ILLEGAL_CONTENT = new ReadOnlyCollection<string> (new List<string>() { NEWLINE });

        public const string SPACE = " ";
        public const string DB_TESTS = "DBT";
        private static readonly IList<string> ILLEGAL_STARTERS = new ReadOnlyCollection<string> (new List<string>() { SPACE, DB_TESTS });
        ...
        public const string DB_ERROR = "Cannot write changes to data base - changes were not saved.";
        ...
        public static bool isValidEmail(string eMail)
        {
            ...
            if (eMailSuffix.Contains("@") || !eMailSuffix.Contains(".") || eMailSuffix.IndexOf('.') == 0 || 
                eMailSuffix.IndexOf('.') == eMailSuffix.Length - 1 || eMailSuffix.LastIndexOf('.') == eMailSuffix.Length - 1)
            {
                return false;
            }
            return true;
        ...
        public static bool isValidInput(List<string> input)
        {
            ...
            foreach (string str in input)
            {
                foreach (string bad in ILLEGAL_STARTERS)
                {

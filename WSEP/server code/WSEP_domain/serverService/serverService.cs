...
namespace serverService
{
    public class serverService : IserverService
    {
         ...
         public serverService(bool test)
        {
         ...
         public string parseMessage(string str, IObserverable co)
         {
             serverMessage message = serializer.Deserialize<serverMessage>(str);
             serverMessage ans = parseServerMessage(message, co);
             return serializer.Serialize(ans);
         }

         public Tuple<string, string, List<Tuple<string, string>>> parseMessage2(string str, IObserverable co)
         {
             serverMessage message = serializer.Deserialize<serverMessage>(str);
             serverMessage ans = parseServerMessage(message, co);
         ...
         private serverMessage parseServerMessage(serverMessage message, IObserverable co)
         {
                    ...
                    {
                        case serverMessage.messageType.Register:
                            // implement
                            break;
                        ...
                        case serverMessage.messageType.registerMemberToForum:
                           ...
                           // Console.WriteLine("got to register member");
                            ...
                            ans = forumSystem.registerMemberToForum(forumName, user, password, mail, "q", "a");
                            if (ans.Equals("true"))
                            {
                                return new serverMessage(serverMessage.messageType.success, strLst, intLst, date);
                            }
                        ...
                        case serverMessage.messageType.numOfForums:
                           ...
                           number = forumSystem.getNumOfForums(user);
                           if (number != -1)
                           {
                               intLst.Add(number);
                               return new serverMessage(serverMessage.messageType.success, strLst, intLst, date);
                       ...
                       /* case serverMessage.messageType.ForumsByUser:
                        ...
                        case serverMessage.messageType.login:
                           ...
                           user = message.stringContent.ElementAt(1);
                           password = message.stringContent.ElementAt(2);
                           loginStatus ls = forumSystem.login(forumName, user, password, co);
                           number = 1;
                           switch (ls)
                           {
